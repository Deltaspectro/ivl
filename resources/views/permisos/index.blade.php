...
<div class="col-md-5">
	<table class="table table-hover table-striped">
			...
			<tbody>
				<tr v-bind="permiso in permisos">
					<td width="10px">@{{ permiso.id }}</td>
					<td>@{{ permiso.name	 }}</td>
					<td width="10px">
		...
		<nav>
	      <ul class="pagination">
	        ...
	        <li v-bind="page in pagesNumber" v-bind:class="[ page == isActived ? 'active' : '']">
		...
		methods:{
			getPermiso: function(page) {
                ...
                axios.get(urlUsers).then(response => {
                  this.permisos = response.data.permisos.data,
